...
# Словарь скриптов
scripts = {
    "work_place.sh":                      "Проект меню work_place (Эта программа)",
    "start_work_tree_node.sh":            "Проект рабочего дерева узлов",
    "start_work_folder_site.sh":          "Проект рабочей папки сайта",
    "start_comment_sunpp.sh":             "Проект комментариев SUNPP",
    "docker_simple_docs.sh":              "Простая документация Docker",
    "claster_work_start.sh":              "Проект работы кластера",
    "start_install_sunpp_2vm.sh":         "Проект установки SUNPP на 2 VM",
    "start_install_sunpp_2_vm-docker.sh": "Проект установки SUNPP на 2 VM с Docker",
    "start_multi_dockers.sh":             "Проект запуска неск. Docker контейнеров",
    "start_multi_vm_gen_db_view_repo.sh": "Проект запуска неск. VM с БД и смотром репозитория",
    "start_for_ekatra_test.sh":           "Проект теста для Ekatra",
    "start_gbv_mongodb.sh":               "Проект с генератором, MongoDB и вьювером",
    "start_ekatra_flask.sh":              "Проект сайта ekatra_flask",
    "start_work_rabbitmq.sh":             "Проект с rabbitmq",
    "start_dbv_postgres.sh":              "Проект с dbv_postgres",
    "start_db_tlds_timescaledb.sh":       "Проект db_tlds_timescaledb",
    "start_db_tlds_questdb.sh":           "Проект db_tlds_questdb",
    "start_compose_go.sh":                "Проект compose with go - start_compose_go",
    "start_ivldm.sh":                     "Проекта IVLDM (сайт IVL document manager) на основе SQLite",
    "start_quesrdb_test.sh":              "Проект теста работы questdb вместе с app.py",
    "start_app_to_postgres.sh":           "Проект работы postgresql в контейнере на уд. VM(Vital)",
    "start_html_bootstrap_js.sh":         "Проекта с сервером сайта на bootstrap",
}
...
def draw_menu(stdscr, selected_row_idx, history, history_idx, history_date_idx, active_column):
    ...
    about_text = [
        "------------------------------------------------------------",
        "Программа запуска work-userspace",
        "author: Старых ЛН",
